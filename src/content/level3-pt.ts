// Single source of truth for details that may change.
// When the start date is confirmed, update START_DATE here and it updates everywhere.

export const START_DATE = '27th September 2026';

export const BOOKING_URL = 'https://ptqualificationdiploma.info/tphealth-pt';
export const MAIN_SITE_URL = 'https://www.tphealthfitness.com';
export const COURSE_PAGE_URL = 'https://www.tphealthfitness.com/level-3-pt-course/';
export const INSTAGRAM_URL = 'https://www.instagram.com/tphealthandfitness_/';

export const CONTACT = {
  email: 'teighlor@tphealthfitness.com',
  phone: '07577 409 054',
  phoneHref: 'tel:+447577409054',
  instagramHandle: '@tphealthandfitness_',
};

export const ENQUIRY_ENDPOINT = '/api/enquiry.php';
